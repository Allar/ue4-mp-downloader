...
epic_api.prototype.DownloadItemChunkList = function (manifest, chunkList, downloadDirBase, cb) {
	...
	var chunkSize = 1000;
	var downloader = function (chunkList, cb) {
		var downloads = [];
	...
	var i = 0;
	var chunkCallback = function (inc) {
		var downloadArr;
		i += inc;
		if (i < chunkList.length) {
			var limit = Math.min(i+chunkSize, chunkList.length);
			console.log(`Downloading item chunks ${i}..${limit} of ${chunkList.length}`);
	    downloadArr = chunkList.slice(i,limit);
	    downloader(downloadArr, chunkCallback);
		} else {
			cb(true, downloadDir);
		}
	}
